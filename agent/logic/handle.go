...
// 玩家登陆过程
func H_login_req(ss *Session) ([]byte, error) {
	// TODO: 登陆鉴权
	...
	ss.GSID = DEFAULT_GSID

	//// 连接到已选定GAME服务器
	//conn := sp.GetServiceWithId(sp.DEFAULT_SERVICE_PATH+"/game", ss.GSID)
	//if conn == nil {
	//	log.Error("cannot get game service:", ss.GSID)
	//	return nil
	//}
	cli := pb.NewGameServiceClient(services.GetGameConn())

	// 开启到游戏服的流
	ctx := metadata.NewContext(context.Background(), metadata.New(map[string]string{"userid": fmt.Sprint(ss.UserId)}))
	...
	fetcherTask := func(ss *Session) {
		for {
			in, err := ss.Stream.Recv()
			if err == io.EOF { // 流关闭
				log.Debug(err)
				return
			}
	...
	if ret, err := proto.Marshal(&agentpb.LoginAck{UserId: ss.UserId}); err != nil {
		log.Error(err)
		return nil, nil
	} else {
